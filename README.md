...
- **面板** — 本地网关的运行状态、调用统计、自动切换情况，以及应用/模型总览（模型概览最多列 5 个）
...
- **自动切换**：模型列表右上角的开关。开启后，网关向上游发起请求失败时
  ...
  成功后继续本次请求，并把这个接手成功的模型记为当前模型 —— 列表的「使用中」
  随之前移，后续请求也以它为首选，不必每次都先撞一遍已失败的主模型。
  面板会显示已触发次数与最近一次 `X → Y` 的切换记录。
- **按请求里的模型名路由**：入站请求的 `model` 传 `aiStart` 或 `auto`（不区分大小写）时走上面的逻辑；
  传「模型」页里某个模型的**显示名称**（不区分大小写）时，只调用那一个模型 ——
  即使开着自动切换也不会切换。两者都不命中时同样退回上面的逻辑。
